...
class McpToolHandler
{
    ...
    private function evaluateResult(array $args, ?User $user = null): array
    {
        ...
        if (! empty($args['provider'])) {
            $providerOverride = LlmProvider::where('name', 'like', $args['provider'])
                ->where('is_active', true)
                ->first();
            if (! $providerOverride) {
    ...
    private function storeEvaluation(array $args, ?User $user = null): array
    {
        ...
        $composite = ResultEvaluation::compositeScore($resultId, $evalVersion);
    ...
    private function getEvaluationPrompt(array $args, ?User $user = null): array
    {
        ...
        $promptSlug = EvaluationSetting::get('prompt_slug', 'system-evaluation-template');
        $prompt = Prompt::where('slug', $promptSlug)->first();
    ...
    private function updateChannelTool(array $args, ?User $user): array
    {
        ...
        $updates = [];
        if (isset($args['role_label'])) {
            $updates['role_label'] = $args['role_label'];
        }
        if (isset($args['system_prompt'])) {
