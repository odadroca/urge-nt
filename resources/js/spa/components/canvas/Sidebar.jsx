...
export default function Sidebar({ visible, nodes, layoutMode, onLayoutChange, activeLayers, onToggleLayer, hiddenNodes, onToggleNodeVisibility, onToggleAllVisibility }) {
...
function Section({ title, nodeType, items, onDragStart, hiddenIds, onToggleVisibility, onToggleAll }) {
    const allHidden = hiddenIds && items.length > 0 && items.every(n => hiddenIds.has(n.id));

    return (
        <div>
            <div className="flex items-center justify-between mb-1.5">
                <span className="text-[9px] text-gray-500 uppercase tracking-wider">{title}</span>
                {onToggleAll && (
                    <button
                        onClick={() => onToggleAll(nodeType)}
